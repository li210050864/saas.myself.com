...
class UploadFile{
	...
	protected $error_messages = array(
		...
		'max_file_size' => 'The file too big',
		'min_file_size' => 'The file too small',
	...
	public function __construct($options = null,$initialize = true,$error_message = null){
		$this->response = array();
		$this->response = array(
			...
			'redirect_allow_targer' => '/^'.preg_quote(
				parse_url($this->get_server_var('HTTP_REFERER'),PHP_URL_SCHEME).'://'.parse_url($this->get_server_var('HTTP_REFERER'),PHP_URL_HOST).'/','/'
			).'/',
	...
	protected function get_error_message($error){
		return isset($this->error_message[$error]) ? $this->error_messages[$error] : $error;
	}
	...
	protected function validate($uploaded_file,$file,$error,$index){
		...
		$post_max_size = $this->get_config_bytes(init_get('post_max_size'));
		if($post_max_size && ($content_length > $post_max_size)){
			$file->error = $this->get_error_message('post_max_size');
			return false;
		}
		if($upload_file && is_uploaded_file($uploaded_file)){
			$file_size = $this->get_file_size($uploaded_file);
		}else{
			$file_size = $content_length;
		}
		if($this->options['max_file_size'] && ($file_size > $this->options['max_file_size'] || $file_size > $this->options['max_file_size'])){
			$file->error = $this->get_error_message('max_file_size');
			return false;
		}
		if($this->options['min_file_size'] && $file_size < $this->options['min_file_size']){
			$file_error = $this->get_error_message('min_file_size');
			return false;
		}
		if(is_int($this->options['max_number_of_files']) && ($this->count_file_objects() >= $this->options['max_number_of_files']) && !is_file($this->get_upload_path($file->name))){
			$file->error = $this->get_error_message('max_numerb_of_files');
			return false;
		}
		$max_wdith = @$this->options['max_width'];
		$max_height = @$this->options['max_height'];
		$min_width = @$this->options['min_width'];
		$min_height = @$this->options['min_height'];

		if($max_width || $max_height || $min_widht || $min_height && preg_match($this->options['image_file_types'],$file->name)){
			list($img_width,$img_height) = $this->get_image_size($uploaded_file);
			if(
				@$this->options['image_versions']['']['auto_orient'] && function_exists('exif_read_data') && 
